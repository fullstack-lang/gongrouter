...
import (
	...
	gongrouter_go "github.com/fullstack-lang/gongrouter/go"
	...
	gongrouter_static "github.com/fullstack-lang/gongrouter/go/static"
...
func (impl *BeforeCommitImplementation) BeforeCommit(stage *gongrouter_models.StageStruct) {
	...
	stage.Checkout()
	stage.Marshall(file, "github.com/fullstack-lang/gongrouter/go/models", "main")
}

func main() {

	log.SetPrefix("gongrouter: ")
	log.SetFlags(0)
	...
	r := gongrouter_static.ServeStaticFiles(*logGINFlag)

	// setup stack
	var stage *gongrouter_models.StageStruct
	if *marshallOnCommit != "" {
		// persistence in a SQLite file on disk in memory
		stage = gongrouter_fullstack.NewStackInstance(r, "gongrouter")
	} else {
		// persistence in a SQLite file on disk
		stage = gongrouter_fullstack.NewStackInstance(r, "gongrouter", "./test.db")
	}
	...
	if *unmarshallFromCode != "" {
		...
		stage.Reset()
		stage.Commit()
		err := gongrouter_models.ParseAstFile(stage, *unmarshallFromCode)
	...
	gongdoc_load.Load(
		...
		gongrouter_go.GoModelsDir,
		gongrouter_go.GoDiagramsDir,
		r,
		*embeddedDiagrams,
